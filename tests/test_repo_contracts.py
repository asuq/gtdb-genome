...
def test_runtime_docs_match_current_readme_and_usage_details() -> None:
    ...
    assert_contains_all(
        ...
        (
            ...
            "leftover GTDB-genomes run artefacts",
            "-r, --gtdb-release",
            "-j, --threads",
            "-d, --dry-run",
            "--version-latest",
    ...
    assert_contains_all(
        ...
        (
            ...
            "GTDB-genomes artefacts from a previous run",
            ".gtdb_genomes_work/",
            ...
            "workflow retries run in waves",
            "every batch in the current wave",
            "suppressed-only direct groups get 2 total workflow",
            "non-suppressed accession gets 4 total",
            "execution order, not a fixed retry cap",
            "real runs log",
            "10% progress milestones for direct downloads",
            "dehydrated batch downloads",
...
def test_real_data_validation_guide_describes_local_requirements() -> None:
    ...
    assert_contains_all(
        ...
        (
            ...
            "C8` installs a case-local `datasets` wrapper",
            "leaves `NCBI_API_KEY` ambient",
            "Required environment for `full-large` coverage:",
            "packaged-runtime `C` coverage is split into separate build and clean-runtime",
...
def test_ci_workflow_runs_expected_validation_suites() -> None:
    ...
    assert_contains_all(
        ...
        (
            ...
            "bin/run-real-data-tests-remote.sh C1 C2 C3 C4 C5 C6 C8",
            "uv build",
