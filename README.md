...
`gtdb-genomes` is available on PyPI and can be installed with conda (bioconda):
...
- `--release`: Accepts bundled aliases such as `latest`, `80`, `95`, `214`, `226`, `220.0`,
or `release220/220.0`.
...
- `--taxon`: Repeatable. A row is selected when its GTDB lineage contains the requested GTDB
token exactly after trimming. Matching is case-sensitive.


### Optional options

- `--output`: Out put directory must either not exist or exist as an empty directory. The
tool does not merge into or overwrite a populated output tree.

- `--prefer-genbank`: Enabled by default. When a requested accession is `GCF_*`, the tool inspects
...
Prefer paired GenBank accessions and request extra annotation with NCBI API key:
...
> [!IMPORTANT]
> `--ncbi-api-key` expects an NCBI API key. The tool passes it only to the upstream `datasets` command and does not use it for GTDB release resolution, local taxonomy loading, or any other service.
>
> The tool forwards `--ncbi-api-key` to `datasets --api-key` without writing it to project files.
>
> It:
...
If a paired GenBank accession is unavailable or metadata lookup exhausts its retry budget, the original accession is kept.
